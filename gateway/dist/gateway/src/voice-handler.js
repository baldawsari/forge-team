...
// Voice Handler
// ---------------------------------------------------------------------------
class VoiceHandler extends eventemitter3_1.EventEmitter {
    config;
    /** Track processing state */
    activeTranscriptions = new Set();
    activeSyntheses = new Set();
    constructor(config) {
        super();
    ...
    async transcribe(audioBuffer, language) {
        ...
        try {
            ...
            if (audioBuffer.length > maxBytes) {
                throw new Error(`Audio too long: ${audioBuffer.length} bytes exceeds max ${maxBytes} bytes (${this.config.maxDurationSeconds}s)`);
            }
            // Build multipart form data for Whisper API
            const boundary = `----FormBoundary${(0, uuid_1.v4)().replace(/-/g, '')}`;
            const formParts = [];
            // File field
